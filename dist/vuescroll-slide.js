...
(function (global, factory) {
    ...
    methods: {
      ...
      scrollIntoView: function scrollIntoView(elm) {
        ...
        if (!isChildInParent(elm, parentElm)) {
          warn$1('The element or selector you passed is not the element of Vuescroll, please pass the element that is in Vuescroll to scrollIntoView API. ');
          return;
        } // parent elm left, top
  ...
  var api$1 = {
    ...
    methods: {
      ...
      zoomBy: function zoomBy(factor, animate, originLeft, originTop, callback) {
        if (!this.scroller) {
          warn$2('zoomBy and zoomTo are only for slide mode!');
          return;
        }
      ...
      getCurrentPage: function getCurrentPage() {
        if (!this.scroller || !this.mergedOptions.vuescroll.paging) {
          warn$2('getCurrentPage and goToPage are only for slide mode and paging is enble!');
          return;
        }
      ...
      triggerRefreshOrLoad: function triggerRefreshOrLoad(type) {
        if (!this.scroller) {
          warn$2('You can only use triggerRefreshOrLoad in slide mode!');
          return;
        }
        ...
        if (type == 'refresh' && !isRefresh) {
          warn$2('refresh must be enabled!');
          return;
        } else if (type == 'load' && !isLoad) {
          // eslint-disable-next-line
          warn$2("load must be enabled and content's height > container's height!");
          return;
        } else if (type !== 'refresh' && type !== 'load') {
          warn$2('param must be one of load and refresh!');
          return;
        }
        /* istanbul ignore if */
    ...
    mounted: function mounted() {
      ...
      this.$nextTick(function () {
        if (!_this._isDestroyed && !_this.renderError) {
          ...
          _this.$watch('mergedOptions.vuescroll.scroller.disable', function (newVal) {
            if (_this.scroller) {
    ...
    methods: {
      ...
      registryResize: function registryResize() {
        ...
        var handleDomResize = function handleDomResize() {
          ...
          _this2.$forceUpdate(); // Since content sie changes, we should tell parent to set
  ...
  var Vuescroll = _objectSpread2({}, _install(core$1, createPanel, configs, configValidator));
